    ...
    <tr>
        <td colspan='8'>
            <form action="<?php echo $this->createUrl('index') ?>" method='get'>
                <a href="<?php echo $this->createUrl('create') ?>" class='bta'>添加比价产品</a>
                <span id='delall' class='bta' onclick='delall()'>删除</span>
                <input type='text' placeholder="产品名称" name='name' value="<?php echo Yii::app()->request->getParam('name','') ?>" style='height:27px;margin:0 10px'  />
                <input type='submit'  class='bt3' value='搜索' />
            </form>
        </td>
    </tr>
        <tr><td>ID</td><td><input type='checkbox' id='checkAll' myid='0'  />选择</td><td>用户</td><td>类型</td><td>名称</td><td>创建日期</td><td>操作</td></tr>
        <?php foreach($model as $k=>$v): ?>
            <?php $type = $v->type;$type_name = $type.'_name'; ?>
            <tr class='fortr'>
                <td width='5%'><?php echo $v->id ?></td>
                 <td width='7%'><input type='checkbox' name='del' myid='<?php echo $v->id ?>' /></td>
                <td><?php echo $v->user_id == 0 ? '匿名' : $v->user->username ?></td>
                <td><?php echo Favorite::model()->getType($type) ?></td>
