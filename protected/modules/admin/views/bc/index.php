<table class='table'>
    <tr>
        <td colspan='6'>
            <form action="<?php echo $this->createUrl('index') ?>" method='get'>
                <a class='bta' href="<?php echo $this->createUrl('create') ?>" >添加商圈</a>
                <span id='delall' class='bta' onclick='delall()'>删除</span>
                <input type='text' placeholder="商圈名称" name='bc_name' value="<?php echo Yii::app()->request->getParam('bc_name') ?>" style='height:27px;margin:0 10px' />
                <?php 
    ...
    <?php foreach ($model as $k => $v): ?>
        <tr class='fortr'>
            ...
            <td><?php echo $v->bc_name ?></td>
            <td><?php echo $v->city->city_name; ?></td>
            <td><?php echo empty($v->image) ? '否' : '是'; ?></td>
            <td>
                <a href="__ROOT__/index.php/商圈/show/商圈_ID/{$v.商圈_ID}/Item_ID/{$v.Area_ID}" target='_blank'>[预览]</a>&nbsp;&nbsp;
...
<form id='form1' action="<?php echo $this->createUrl('delete') ?>" method="post">
    <input type='hidden' name='bc_id' id='myids'  />
</form>