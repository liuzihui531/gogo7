...
<table class="table">
    ...
    <tr>
        ...
        <td>
            <div id='main_seller' style="background: #EBEBE4;width:800px;min-height:150px;border:1px solid #A9A9A9">
                <?php if ($isEdit && isset($model->sellers) && !empty($model->sellers)): ?>
                    <?php foreach ($model->sellers as $k => $v): ?>
                        <span id='seller_text_<?php echo $v->id ?>' style='padding:5px'><?php echo $v->seller_name ?></span>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>&nbsp;&nbsp;
            <br/><input type='text' style='width:160px' id="search_seller_name" name='seller_name' placeholder="请输入检索内容" />&nbsp;&nbsp;
            <input type='button' class='bta' value='检索' id='searchseller' /><span class='field_desc'>必须在多选框中选择，直接输入无效</span>
            <div class='main_seller'>
                <p style='float:right;margin:0px 5px 5px 0px;padding:0'><span class='close'>关闭</span></p>
                <p style='clear:both' id='seller_checkbox'>
                    <!--这里是搜索后的内容-->
    ...
    <tr>
     ...
     <tr>
        <td align='right'><?php echo $form->labelEx($model, 'start_time') ?></td>
        <td>
            <?php echo $form->dateField($model,'start_time'); ?>
        </td>
    </tr>
    <tr>
        <td align='right'><?php echo $form->labelEx($model, 'expire_time') ?></td>
        <td>
...
<div id='seller_checkboxhidden'>
    <?php if ($isEdit && isset($model->sellers) && !empty($model->sellers)): ?>
        <?php foreach ($model->sellers as $k => $v): ?>
            <input type="hidden" name="seller_id[]" value="<?php echo $v->id ?>" />
        <?php endforeach; ?>
    <?php endif; ?>
</div>
<?php $this->endWidget(); ?>
