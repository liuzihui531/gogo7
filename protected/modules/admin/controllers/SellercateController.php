...
/**
 * Description of SellercateController
 *
 * @author lzh
 */
class SellercateController extends AdminControllerD {
    ...
    public function actionIndex() {
        ...
        $model = Sellercate::model()->findAll($criteria);
        $this->render('index', array('model' => $model));
    }

    public function actionCreate() {
        $this->breadcrumbs = array('添加分类管理');
        $model = new Sellercate();
        $this->render('_form', array('model' => $model));
    }

    public function actionUpdate($id = 0) {
        $this->breadcrumbs = array('修改分类管理');
        $model = Sellercate::model()->findByPk($id);
    ...
    public function actionSave() {
        ...
        if (isset($_POST['Sellercate'])) {
            $model->attributes = $_POST['Sellercate'];
            if ($model->save()) {
    ...
    public function actionUpdateSort() {
        ...
        foreach ($id as $k => $v) {
            ...
            Yii::app()->db->createCommand()->update('sellercate', $data, 'id=' . $v);
        }
        $this->redirect('index');
    }
    ...
    public function actionDelete() {
        ...
        try {
            ...
            foreach ($model as $k => $v) {
                if(SellercateRelation::model()->find('cate_id=:cate_id',array(':cate_id'=>$v->id))){
                    throw new Exception('您必须把该分类下的商家全部删除才能删除分类！');
                }
                if (!$v->delete()) {
                    throw new Exception('操作有误！');
                }
