 ...
 * @author lzh
 */
class BrandcateController extends AdminControllerD {

    public function actionIndex() {
        $this->breadcrumbs = array('品牌分类管理');
        $criteria = new CDbCriteria();
        if (isset($_GET['name']) || !empty($_GET['name'])) {
            $criteria->addSearchCondition('name', $_GET['name']);
        }
        $criteria->order = 'sorting desc,id desc';
        $model = Brandcate::model()->findAll($criteria);
    ...
    public function actionUpdate($id = 0) {
        ...
        $model = Brandcate::model()->findByPk($id);
    ...
    public function actionUpdateSort() {
        ...
        foreach ($id as $k => $v) {
            ...
            Yii::app()->db->createCommand()->update('brandcate', $data, 'id=' . $v);
        }
        $this->redirect('index');
    }
    ...
    public function actionDelete() {
        ...
        try {
            ...
            foreach ($model as $k => $v) {
                if(BrandcateRelation::model()->find('cate_id=:id',array(':id'=>$v->id))){
                    throw new Exception('您必须把该分类下的品牌全部删除才能删除分类！');
                }
                if (!$v->delete()) {
                    throw new Exception('操作有误！');
                }
